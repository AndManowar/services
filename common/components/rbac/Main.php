...
class Main extends Component
{
    ...
    const GROUP_PERMITSSION_PREFIX = 'GR';
    const ROUTE_PERMITSSION_PREFIX = 'RT';
     ...
     * @return bool
     *
     * Проверка являеться ли текущий пользователь ROOT
     */
    public function isRoot()
    {
        ...
        // Проверка на супервизораы
        $role = Yii::$app->authManager->getRolesByUser($user);
        if (isset($role[self::ROLE_ROOT])) {
            return true;
        } else {
            return false;
        }
    ...
    public function addGroupPermissions($permisions)
    {
        foreach ($permisions as $permision) {
            $name = $this->uniqNamePermission($permision->systemName, self::GROUP_PERMITSSION_PREFIX);
    ...
    {
        $name = $this->uniqNamePermission($description, self::GROUP_PERMITSSION_PREFIX);
        $permitssion = Yii::$app->authManager->createPermission($name);
        $permitssion->description = $description;

        Yii::$app->authManager->add($permitssion);
    }
    ...
    public function editGroupPermission($name, $description)
    {
        ...
        Yii::$app->authManager->update($name, $permitssion);
    }
    ...
    {
        $permitssion = Yii::$app->authManager->getPermission($name);
        Yii::$app->authManager->remove($permitssion);
    }
    ...
    public function getAllGroupPermission()
    {
        ...
        foreach ($list as $key => $item) {
            if (substr($item->name, 0, 2) == self::GROUP_PERMITSSION_PREFIX) {
                $out[$key] = $item;
            }
        }
        return $out;
    ...
    {
        if (isset($this->branchs[$branchId])) {
            $permitssion = Yii::$app->authManager->createPermission($this->parseRoute($route, $branchId));
            Yii::$app->authManager->add($permitssion);
    ...
    {
        return static::ROUTE_PERMITSSION_PREFIX . $this->branchs[$branchId][0] . '/' . $route;
    }
    ...
    public function unParseRoute($route)
    {
        ...
        foreach ($this->branchs as $item) {
            $prefix = substr($route, 0, 3);
            if ($prefix == static::ROUTE_PERMITSSION_PREFIX . $item[0]) {
                $return['branch'] = $item[2];
                $return['rout'] = str_replace(static::ROUTE_PERMITSSION_PREFIX . $item[0], '', $route);
            }
        }
        return $return;
    }
